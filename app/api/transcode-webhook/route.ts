...
function verifyTranscodeWebhookHmacSha256(rawBody: string, headerValue: string | null, secret: string): boolean {
  if (!headerValue || !secret) return false
  const v = headerValue.trim()
  const hex = v.toLowerCase().startsWith("sha256=") ? v.slice("sha256=".length).trim() : v.trim()

  let expectedHex: string
  try {
    expectedHex = createHmac("sha256", secret).update(rawBody, "utf8").digest("hex")
  } catch {
    return false
  }
...
export async function POST(request: Request): Promise<NextResponse> {
  const secret = process.env.TRANSCODER_WEBHOOK_SECRET?.trim()
  if (!secret) {
  ...
  if (!verifyTranscodeWebhookHmacSha256(rawBody, sig, secret)) {
  ...
  if (!masterKey.startsWith(lessonPrefix) || masterKey.includes("..")) {
    return NextResponse.json({ ok: false, error: "bad_master_key" }, { status: 400 })
  }
  if (!masterKey.endsWith("/master.m3u8")) {
    return NextResponse.json({ ok: false, error: "master_suffix" }, { status: 400 })
  }
  ...
  if (!hlsUrl) {
    return NextResponse.json({ ok: false, error: "missing_public_base" }, { status: 500 })
  }

  await adminUpdateVideoLesson(lessonId, { hls_url: hlsUrl, is_published: true })
