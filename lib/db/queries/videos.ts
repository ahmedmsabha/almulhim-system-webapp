...
import { videoLessons, watchProgress } from "@/lib/db/schema"
...
export async function getPublishedVideos(): Promise<VideoLesson[]> {
...
export async function listVideoLessonsForAdmin(opts?: {
  ...
  if (opts?.is_published !== undefined) {
    predicates.push(eq(videoLessons.is_published, opts.is_published))
  }
  if (opts?.is_preview !== undefined) {
    predicates.push(eq(videoLessons.is_preview, opts.is_preview))
  }
  if (opts?.unit) {
...
export async function getVideoWithAccess(id: string, accessToken: string): Promise<VideoLesson> {
  ...
  return withUserDb(accessToken, async (tx) => {
    const rows = await tx
      .select()
      .from(videoLessons)
      .where(and(eq(videoLessons.id, id), eq(videoLessons.is_published, true)))
      .limit(1)
...
export async function getSubscriberVideos(accessToken: string): Promise<VideoLesson[]> {
  const subClaim = jwtDecode<JwtPayload>(accessToken).sub
  if (!subClaim) {
    throw new UnauthorizedError()
  }
...
export async function upsertWatchProgress(
  ...
  const completedFlag = completedExplicit || progressPercent >= 95
  ...
  await withUserDb(accessToken, async (tx) => {
    await tx
      ...
      .values({
        ...
        last_position: Math.round(secondsWatchedOrLastPosition),
        completed: completedFlag,
        last_watched_at: new Date(),
      })
      .onConflictDoUpdate({
        target: [watchProgress.student_id, watchProgress.lesson_id],
        set: {
          progress: Math.min(100, Math.round(progressPercent)),
          last_position: Math.round(secondsWatchedOrLastPosition),
          completed: completedFlag,
    ...
    const rows =
      lessonId ?
        await tx
          ...
          .from(watchProgress)
          .where(
            and(eq(watchProgress.student_id, subClaim), eq(watchProgress.lesson_id, lessonId))
          )
      : await tx
          .select()
          .from(watchProgress)
...
export async function adminUpdateVideoLesson(
  ...
  const [row] = await adminDb
    .update(videoLessons)
    .set(updatePayload as Partial<typeof videoLessons.$inferInsert>)
    .where(eq(videoLessons.id, id))
    .returning()
...
export async function adminDeleteVideoLesson(id: string): Promise<void> {
  await adminDb.delete(videoLessons).where(eq(videoLessons.id, id))
}
