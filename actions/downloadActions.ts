...
export async function initiateDownloadAction(
  ...
  try {
    ...
    const active = await subscriptionIsActiveAdmin(gate.data.id)
    if (!active) {
      return actionFailure("الاشتراك غير نشط لتنزيل هذا الدرس", "SUBSCRIPTION_EXPIRED")
    }
    ...
    if (lesson.is_preview || !lesson.hls_url?.trim()) {
      return actionFailure("هذا المحتوى غير متاح للتحميل", "FORBIDDEN")
    }

    const q = qualityPixels ?? DEFAULT_QUALITY_PX
    const bundle = await presignHlsOfflineBundle(lesson.hls_url, q)
